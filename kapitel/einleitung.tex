...
\begin{itemize}
    \item \textbf{Fehlende Echtzeitdaten} -- Budget-Informationen sind häufig veraltet, da Excel-Dateien manuell aktualisiert werden müssen
    \item \textbf{Manuelle Fehlerquellen} -- Die Dateneingabe und -aggregation über mehrere Tabellenblätter ist fehleranfällig
    \item \textbf{Fehlende Systemintegration} -- Es existiert keine automatische Verbindung zwischen den Excel-Daten und SAP-Systemen wie \gls{calm}
    \item \textbf{Eingeschränkte Nachvollziehbarkeit} -- Änderungen an Budget-Zuordnungen sind schwer nachvollziehbar
    \item \textbf{Heterogene Datenquellen} -- Relevante Daten verteilen sich auf unterschiedliche Quellen (Excel-Sheets, SAP-Zeitbuchungen, Planungsdaten), die manuell zusammengeführt werden müssen
\end{itemize}
...
Ziel dieser Arbeit ist die Konzeption und Implementierung einer Cloud-Middleware auf der SAP \gls{btp}, welche die folgenden Anforderungen erfüllt:
\begin{enumerate}
    \item Heterogene Datenquellen (Excel/SharePoint, perspektivisch \gls{odata}-Services) hinter einer einheitlichen \gls{api} abstrahieren
    \item Budget-Metriken aus verschiedenen Quellen automatisiert aggregieren und berechnen
    \item Durch den Einsatz geeigneter Architekturmuster die Austauschbarkeit des Backends gewährleisten
    \item Den schrittweisen Wechsel von einem Excel-basierten Backend zu service-orientierten Datenquellen mit minimalem Aufwand ermöglichen
\end{enumerate}
...
\begin{itemize}
    \item Welche Architekturmuster eignen sich für die Abstraktion heterogener Datenquellen in einer Cloud-Middleware?
    \item Wie unterscheiden sich ein Excel-basiertes Backend und eine service-orientierte Integration hinsichtlich Wartbarkeit, Erweiterbarkeit und Performance?
    \item Welcher konkrete Implementierungsaufwand ist für den Wechsel zwischen den Backend-Varianten erforderlich?
\end{itemize}
