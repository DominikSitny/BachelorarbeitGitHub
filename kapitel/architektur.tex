...
\begin{tikzpicture}[
    ...
    \node[sublayer, fill=purple!10] (odata) at (3,-1) {SAPAdapter\\(konzeptionell)};

    % Datenquellen
    \node[sublayer, fill=gray!20] (sp) at (-3,-2.5) {SharePoint/Excel};
    \node[sublayer, fill=gray!20, minimum width=5.5cm, minimum height=1.4cm] (calm) at (3,-2.6) {SAP Cloud ALM\\+ SAP S/4HANA};
...
Die oberste Schicht bildet die Präsentationsschicht mit dem Fiori Frontend, das nicht Teil dieser Arbeit ist. Darunter liegt die API-Schicht, die einen \gls{odata} v4 Service über SAP \gls{cap} bereitstellt. Die Service-Schicht kapselt die Geschäftslogik und Berechnungen, während die Adapter-Schicht den konkreten Datenzugriff über den Excel-Parser oder einen SAP-Connector realisiert. Die unterste Ebene bildet die Datenschicht mit SharePoint/Excel oder den SAP-Systemen (\gls{calm} für User Stories, SAP S/4HANA für Zeitbuchungen) als Datenquellen.
...
Die Adapter-Schicht abstrahiert den Zugriff auf die Datenquellen und stellt der Service-Schicht ein einheitliches Datenmodell zur Verfügung -- unabhängig davon, ob die Daten aus einer Excel-Datei oder direkt aus den SAP-Systemen stammen. Die Service-Schicht ruft ausschließlich Methoden der Adapter-Schnittstelle auf (etwa \texttt{loadTeams()}, \texttt{loadUserStories()} oder \texttt{savePSPAssignment()}) und hat keine Kenntnis über die konkrete Datenquelle. Dieses Prinzip der Umkehrung der Abhängigkeit (\textit{Dependency Inversion}) ist ein Kernelement austauschbarer Architekturen \citep{gamma1994}. Die Adapter-Schicht wird in Abschnitt~4.3 im Detail beschrieben.

\subsubsection{Datenschicht}

Die Datenschicht bezeichnet die externen Datenquellen selbst. Im aktuellen Zustand ist dies eine \texttt{Budgetcontrolling.xlsx} auf dem lokalen Dateisystem (bzw. perspektivisch auf SharePoint), die über Power Query mit Daten aus verschiedenen SAP-Systemen befüllt wird: User Stories und geplante Aufwände stammen aus SAP \gls{calm}, während die Zeitbuchungen (CADO) aus dem operativen SAP-System (z.\,B. SAP S/4HANA) exportiert werden. Bei einem späteren Backend-Wechsel würde diese Schicht durch direkte API-Anbindungen an beide SAP-Systeme ersetzt, ohne dass die darüberliegenden Schichten angepasst werden müssen.
...
\begin{tikzpicture}[
    ...
    \node[class, fill=blue!10] (odata) at (3.5,-4) {SAPAdapter\\(konzeptionell)};
...
\subsubsection{SAP-Adapter (konzeptionell)}

Der SAP-Adapter ist die perspektivische Alternative zum Excel-Adapter und würde Daten direkt von den SAP-Systemen abrufen. Er ist im Rahmen dieser Arbeit nicht implementiert, jedoch so konzipiert, dass er das gleiche DataAdapter-Interface erfüllt und somit den Excel-Adapter ohne Änderungen an der Service-Schicht ersetzen kann.

Anstatt eine lokale Excel-Datei zu parsen, würde der SAP-Adapter HTTP-Requests an zwei verschiedene APIs senden: Die OData-Schnittstelle von SAP \gls{calm} liefert User Stories mit ihren geplanten Aufwänden (\gls{etc}), während die API des operativen SAP-Systems (z.\,B. SAP S/4HANA) die Zeitbuchungen (CADO-Daten) für die Actual-Berechnung bereitstellt. Der Adapter aggregiert beide Datenquellen intern und stellt sie über das einheitliche DataAdapter-Interface bereit. Die Authentifizierung erfolgt über den OAuth 2.0 Client Credentials Flow, da die Middleware als Server-zu-Server-Anwendung ohne Benutzerinteraktion agiert. Das Mapping der SAP-Datenstrukturen auf das interne Datenmodell der Middleware übernimmt der Adapter analog zum Excel-Adapter, sodass die Budget-Berechnungslogik identisch bleibt. Der wesentliche Unterschied liegt darin, dass Echtzeitdaten zur Verfügung stünden, da keine manuelle Aktualisierung der Excel-Power-Query mehr erforderlich wäre. Der geschätzte Implementierungsaufwand für diesen Adapter wird in Kapitel~6 evaluiert.

\subsection{Session-Management}

Da die Middleware mehrere Projekte gleichzeitig unterstützt, muss bei jedem Request bekannt sein, auf welches Projekt sich die Anfrage bezieht. HTTP ist grundsätzlich zustandslos -- jeder Request ist unabhängig von vorherigen Requests \citep{fielding2000}. Um dennoch einen projektbezogenen Kontext herzustellen, wurde ein Session-Management entworfen, das den Ablauf in drei Phasen gliedert.

In der ersten Phase fragt der Client die verfügbaren Projekte über \texttt{GET /getProjects()} ab. Die Middleware ermittelt diese dynamisch aus den vorhandenen Projektordnern im Dateisystem, sodass neue Projekte ohne Code-Änderung hinzugefügt werden können. In der zweiten Phase startet der Client eine Session über \texttt{POST /startSession} mit dem gewählten Projektnamen. Die Middleware erzeugt eine eindeutige Session-ID, speichert die Zuordnung von Session zu Projekt im Arbeitsspeicher und gibt die Session-ID an den Client zurück. In der dritten Phase sendet der Client bei allen folgenden Requests die Session-ID im \texttt{x-session-id}-Header mit. Die Middleware validiert die Session, ermittelt das zugehörige Projekt und lädt die entsprechenden Daten. Auflistung~\ref{lst:session-flow} zeigt diesen Ablauf im Überblick.
...
Requests ohne gültige Session-ID werden mit HTTP 401 abgewiesen -- mit Ausnahme von \texttt{getProjects()} und \texttt{startSession}, die naturgemäß ohne Session funktionieren müssen. Sessions werden nach acht Stunden automatisch invalidiert, um verwaiste Einträge zu vermeiden. Diese Architekturentscheidung wird in Abschnitt~4.8 (DE-02) näher begründet.

\subsection{Datenmodell}

Das Datenmodell orientiert sich an den Strukturen der Excel-Datenquelle und wird deklarativ in \gls{cds} definiert. Es folgt den Prinzipien des Domain-Driven Design \citep{evans2003, vernon2013} und bildet die Domänenkonzepte des Budget-Controllings ab. Abbildung~\ref{fig:er-diagramm} zeigt die Beziehungen zwischen den Entitäten.
...
Die Entitäten lassen sich in drei Kategorien einteilen. Die erste Kategorie bilden die \textbf{Stammdaten-Entitäten} (\texttt{Teams}, \texttt{Timeboxes}), die aus dem Prüftabellen-Sheet gelesen werden und als Referenzdaten für die übrigen Entitäten dienen. Die zweite Kategorie umfasst die \textbf{fachlichen Entitäten}, die direkt aus den Excel-Sheets gelesen werden: \texttt{PSPElements} enthält die Budgetplanung pro \gls{psp}-Element und Team, \texttt{UserStories} bildet die User Stories aus \texttt{qry\_CALM\_Data} mit ihren Aufwänden und \gls{psp}-Zuordnungen ab, \texttt{TimeBookings} repräsentiert die Zeitbuchungen aus dem CADO-Sheet und \texttt{PSPAssignments} die manuellen Zuordnungen aus dem \enquote{1. PSP-Zuordnung}-Sheet. Die dritte Kategorie bilden die \textbf{berechneten Entitäten} (\texttt{BudgetOverview}, \texttt{BudgetOverviewByPSP}), die nicht direkt aus der Excel-Datei stammen, sondern zur Laufzeit aus den fachlichen Entitäten aggregiert werden. Sie stellen die Budget-Metriken (Budget, Actual, \gls{etc}, \gls{eac}) pro Team bzw. pro \gls{psp} bereit.

Das zentrale Verbindungselement zwischen den Entitäten ist das \gls{psp}-Element: Es verknüpft User Stories mit ihrem Budget, Zeitbuchungen mit dem zugehörigen Team und manuelle Zuordnungen mit der Budget-Berechnung (vgl. Abbildung~\ref{fig:er-diagramm}). Tabelle~\ref{tab:entities} gibt einen Überblick über alle Entitäten.
...
\toprule
\textbf{Entität} & \textbf{Kategorie} & \textbf{Beschreibung} \\
\midrule
Teams & Stammdaten & 23 Projekt-Teams aus dem Prüftabellen-Sheet \\
Timeboxes & Stammdaten & Projektphasen (Prepare, Explore, Realize, Deploy, Run) \\
PSPElements & Fachdaten & \gls{psp}-Elemente mit zugeordnetem Team und Budget in PT \\
UserStories & Fachdaten & User Stories mit Aufwänden, Status und \gls{psp}-Zuordnung \\
TimeBookings & Fachdaten & Zeitbuchungen aus dem SAP-Export (Stunden pro \gls{psp}) \\
PSPAssignments & Fachdaten & Manuelle User-Story-zu-PSP-Zuordnungen (CRUD-fähig) \\
BudgetOverview & Berechnet & Budget, Actual, \gls{etc} und \gls{eac} aggregiert pro Team \\
BudgetOverviewByPSP & Berechnet & Budget, Actual, \gls{etc} und \gls{eac} aggregiert pro \gls{psp} \\
\bottomrule
\end{tabularx}
\end{table}
...
Ein besonderes Architekturmerkmal ist der PSP-Override-Mechanismus, der das Problem der verzögerten Datenaktualisierung in der Excel-Datenquelle löst. Die User Stories in \texttt{qry\_CALM\_Data} erhalten ihre \gls{psp}-Zuordnung über eine Power-Query-Verbindung in Excel, die manuell aktualisiert werden muss. Wenn ein Benutzer über die \gls{api} eine neue \gls{psp}-Zuordnung erstellt, würde diese Änderung ohne den Override-Mechanismus erst nach einem manuellen Excel-Refresh sichtbar werden -- ein Widerspruch zur Erwartung einer sofort wirksamen \gls{api}-Operation.

Der Mechanismus arbeitet in zwei Phasen. In der Schreibphase wird die neue Zuordnung über \texttt{createPSPAssignment} direkt in das Sheet \enquote{1. PSP-Zuordnung} geschrieben. In der Lesephase -- bei jedem nachfolgenden \gls{api}-Request -- wird eine Lookup-Map aus den manuellen Zuordnungen erstellt, die den Titel der User Story auf das zugewiesene \gls{psp}-Element abbildet. Anschließend werden alle User Stories aus \texttt{qry\_CALM\_Data} durchlaufen: Existiert für eine User Story ein Eintrag in der Lookup-Map, wird ihr \texttt{pspElement}-Feld mit dem manuell zugewiesenen \gls{psp} überschrieben und das Flag \texttt{pspOverride = true} gesetzt, um die Herkunft der Zuordnung nachvollziehbar zu machen.

Die Auswirkung auf die Budget-Berechnung ist unmittelbar: Die \gls{etc}-Berechnung berücksichtigt nur User Stories mit gültiger \gls{psp}-Zuordnung -- Einträge mit \texttt{pspElement = "unassigned"} werden explizit ausgeschlossen. Erhält eine solche User Story durch den Override eine \gls{psp}-Zuordnung, fließt ihr \gls{etc}-Wert in die Aggregation ein, was die Budget-Metriken sowohl pro Team als auch pro \gls{psp}-Element sofort verändert. Auflistung~\ref{lst:psp-override} zeigt den Ablauf im Überblick.
...
Architektonisch ist dieser Mechanismus ein Beispiel für das Prinzip der Datenhoheit auf Anwendungsebene: Die Middleware übernimmt die Verantwortung für die Konsistenz der Daten, anstatt sich auf die Aktualisierungsmechanismen der Datenquelle zu verlassen. Bei einem späteren Wechsel auf den SAP-Adapter entfällt der Override-Mechanismus, da Änderungen direkt in den SAP-Systemen persistiert und über deren Schnittstellen in Echtzeit bereitgestellt würden.
...
Abbildung~\ref{fig:seq-session} zeigt den typischen Ablauf einer Budget-Abfrage. Der Client startet zunächst eine Session, wobei die API-Schicht den Request an die Service-Schicht weiterleitet, die eine Session-ID erzeugt und zurückgibt. Beim anschließenden Datenabruf validiert die Service-Schicht die Session, delegiert das Laden der Rohdaten an die Adapter-Schicht und führt die Budget-Berechnung (\texttt{calculateMetrics}) innerhalb der Service-Schicht durch, bevor das Ergebnis als JSON zurückgegeben wird. Die Berechnung erfolgt bei jedem Request neu, da die Middleware keinen Cache verwendet (vgl. DE-01).
...
Abbildung~\ref{fig:seq-assignment} zeigt den Ablauf beim Erstellen einer \gls{psp}-Zuordnung. Nach der Session-Validierung prüft die Service-Schicht die Eingabeparameter und erkennt potenzielle Team-Mismatches, bevor die Adapter-Schicht den Schreibvorgang in die Excel-Datei ausführt. Die Response enthält neben dem Erfolgsstatus auch eventuelle Warnungen, sodass der Client auf Team-Mismatches hinweisen kann.
...
Die Middleware implementiert eine einheitliche Fehlerbehandlung, die sich an den HTTP-Statuscodes orientiert und das von SAP \gls{cap} vorgegebene OData-Fehlerformat nutzt. Anstatt Fehler als reguläre Responses mit \texttt{success: false} zurückzugeben, werden sie über die \gls{cap}-eigene Methode \texttt{req.error()} signalisiert. Das Framework transformiert diese automatisch in OData-konforme Fehler-Responses mit dem korrekten HTTP-Statuscode, sodass Clients die Fehler einheitlich auswerten können. Tabelle~\ref{tab:error-handling} zeigt die verwendeten Statuscodes und ihre Szenarien.
...
Die Fehlerbehandlung folgt dem Prinzip, Fehler so früh wie möglich zu erkennen: Die Session-Validierung wird vor der eigentlichen Geschäftslogik ausgeführt, sodass ungültige Requests keine unnötigen Datenzugriffe auslösen. Fehler werden als JSON-Objekt mit \texttt{error}-Feld zurückgegeben (Auflistung~\ref{lst:error-response}):
...
Die Sicherheit der Middleware basiert auf mehreren Ebenen, die zusammen einen Defense-in-Depth-Ansatz bilden \citep{owasp2021}.

\subsubsection{Authentifizierung und Autorisierung}

Die Benutzerauthentifizierung wird durch den SAP Authorization and Trust Management Service (XSUAA) auf der \gls{btp} bereitgestellt und ist nicht Teil der Middleware-Implementierung. Die Middleware setzt eine gültige Authentifizierung voraus und verwaltet lediglich die Session-Zuordnung zu Projekten. In einer produktiven Umgebung würden zusätzlich rollenbasierte Berechtigungen (RBAC) implementiert, um zu steuern, welche Benutzer \gls{psp}-Zuordnungen erstellen oder löschen dürfen.

\subsubsection{Input-Validierung}

Alle Eingabeparameter werden vor der Verarbeitung validiert, um Injection-Angriffe zu verhindern:

\begin{itemize}
    \item \textbf{PSP-Zuordnungen}: Die Parameter \texttt{userStory}, \texttt{psp}, \texttt{teamUserStory} und \texttt{teamPSP} werden auf Typ (String), Länge (max. 200 Zeichen) und unzulässige Zeichen geprüft.
    \item \textbf{Session-IDs}: Nur alphanumerische Zeichen werden akzeptiert; die ID wird gegen die In-Memory-Map validiert, nicht gegen eine Datenbank (kein SQL-Injection-Risiko).
    \item \textbf{Projektnamen}: Werden gegen die Whitelist der vorhandenen Projektordner geprüft, um Path-Traversal-Angriffe zu verhindern.
\end{itemize}

\subsubsection{Sichere Kommunikation}

Die Kommunikation zwischen Client und Middleware erfolgt ausschließlich über HTTPS, was durch die SAP \gls{btp} Cloud Foundry Infrastruktur erzwungen wird. Die Authentifizierung gegenüber Microsoft Graph (für SharePoint-Zugriff) nutzt OAuth 2.0 mit Client Credentials, wobei das Client-Secret als Umgebungsvariable auf der \gls{btp} hinterlegt ist und nicht im Quellcode erscheint.

\subsubsection{Limitierungen}

Die aktuelle Implementierung weist folgende sicherheitsrelevante Limitierungen auf, die für den produktiven Einsatz adressiert werden sollten:

\begin{itemize}
    \item Keine Rate-Limiting-Mechanismen gegen DoS-Angriffe
    \item Kein Audit-Trail für Änderungen an \gls{psp}-Zuordnungen
    \item Session-IDs werden nicht kryptographisch sicher generiert (für Prototyp akzeptabel)
\end{itemize}
