...
Die Service-Schicht kapselt die Geschäftslogik. Dazu gehören die Berechnung der Budget-Metriken (Budget, Actual, \gls{etc}, \gls{eac}), die Aggregation pro Team und pro \gls{psp}, die Validierung von \gls{psp}-Zuordnungen sowie die Generierung von Warnungen bei Team-Mismatches.

\subsubsection{Adapter-Schicht}

Die Adapter-Schicht abstrahiert den Zugriff auf die Datenquellen. Sie implementiert das Adapter-Pattern und ermöglicht den Austausch des Backends, ohne dass Änderungen an der Service-Schicht erforderlich sind.
...
Der Excel-Adapter liest Daten aus der \texttt{Budgetcontrolling.xlsx} und schreibt Änderungen zurück. Er ist verantwortlich für das Parsen der verschiedenen Sheets, das Mapping auf das einheitliche Datenmodell sowie den Write-Back für \gls{psp}-Zuordnungen.

\subsubsection{OData-Adapter (konzeptionell)}

Der OData-Adapter würde Daten von SAP \gls{calm} Services abrufen. Dies umfasst die Authentifizierung via OAuth 2.0, den Abruf von User Stories und Zeitbuchungen sowie das Mapping auf das einheitliche Datenmodell.
