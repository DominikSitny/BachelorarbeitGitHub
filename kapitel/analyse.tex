...
\begin{tikzpicture}[
    ...
    % Datenquellen links
    \node[box, fill=blue!20] (calm) at (0,2.2) {SAP Cloud ALM\\(User Stories)};
    \node[box, fill=blue!10] (s4) at (0,0.8) {SAP S/4HANA\\(CADO-Daten)};
    \node[box, fill=green!20] (sharepoint) at (0,-0.6) {SharePoint\\Excel};

    % Middleware Mitte
    \node[box, fill=yellow!30, minimum width=3.5cm, minimum height=3.5cm] (middleware) at (5,0.8) {BC Middleware\\(SAP CAP/BTP)};

    % Frontend rechts
    \node[box, fill=orange!20] (frontend) at (10,0.8) {Fiori\\Frontend};

    % Pfeile
    \draw[arrow] (calm) -- (middleware);
    \draw[arrow] (s4) -- (middleware);
...
Die Problemstellung der vorliegenden Arbeit berührt mehrere Forschungsbereiche, die im Folgenden eingeordnet und voneinander abgegrenzt werden.
...
Im SAP-Ökosystem existieren verschiedene Integrationsansätze: Die SAP Integration Suite bietet vorgefertigte Konnektoren für gängige Systeme \citep{sap2024btp}, erfordert jedoch zusätzliche Lizenzkosten und eignet sich primär für Standard-Integrationsszenarien. \cite{keller2019} beschreiben die Architektur von SAP \gls{cap} und dessen Eignung für die Entwicklung von Microservices, wobei der Fokus auf der Datenbankanbindung liegt -- die Abstraktion heterogener Datenquellen wird nicht thematisiert.

\subsubsection{Spreadsheet-basierte Systeme in Unternehmen}

Die Verbreitung und Risiken von Spreadsheet-Anwendungen in Unternehmen sind Gegenstand umfangreicher Forschung. \cite{panko2016} beschreiben Tabellenkalkulationen als \enquote{Dark Matter} der Unternehmens-IT -- allgegenwärtig, aber schwer zu kontrollieren. \cite{powell2009} dokumentieren in ihrer Metastudie, dass Spreadsheets häufig Fehler enthalten und dennoch für geschäftskritische Entscheidungen verwendet werden. \cite{hermans2015} entwickelten Werkzeuge zur Analyse und Verbesserung von Spreadsheet-Qualität, adressieren jedoch nicht die Migration zu service-orientierten Architekturen.

Diese Erkenntnisse unterstreichen die Relevanz von Lösungen, die einen kontrollierten Übergang zu strukturierteren Systemen ermöglichen, ohne die vertraute Excel-Arbeitsweise abrupt zu beenden.

\subsubsection{Low-Code-Plattformen als Alternative}

Ein alternativer Ansatz zur Ablösung von Excel-basierten Lösungen sind Low-Code-Plattformen wie SAP Build Apps oder Microsoft Power Platform \citep{sahay2020}. Diese ermöglichen die schnelle Entwicklung von Anwendungen mit minimalem Programmieraufwand. Allerdings bieten sie weniger Kontrolle über die Architektur und erschweren die Integration in bestehende Entwicklungsprozesse \citep{bock2021}. Die vorliegende Arbeit verfolgt bewusst einen Pro-Code-Ansatz mit SAP \gls{cap}, um maximale Flexibilität bei der Backend-Austauschbarkeit zu gewährleisten.

\subsubsection{Abgrenzung}

Im Unterschied zu bestehenden Arbeiten, die sich primär auf die Fehleranalyse von Spreadsheets, allgemeine Integrationsarchitekturen oder Low-Code-Alternativen konzentrieren, adressiert diese Arbeit die spezifische Fragestellung der Backend-Austauschbarkeit im SAP-Kontext. Der Fokus liegt auf der praktischen Umsetzung einer Architektur, die sowohl Excel-basierte als auch service-orientierte Backends unterstützt -- ein Szenario, das in der wissenschaftlichen Literatur bislang nicht systematisch untersucht wurde.
