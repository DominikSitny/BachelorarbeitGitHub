...
In diesem Kapitel werden die theoretischen Grundlagen erläutert, die für das Verständnis der entwickelten Middleware-Architektur erforderlich sind. Zunächst werden die verwendeten SAP-Technologien vorgestellt (Abschnitt~2.1--2.2), anschließend das OData-Protokoll (Abschnitt~2.3), dann relevante Architekturmuster (Abschnitt~2.4) und abschließend die Microsoft Graph API für den SharePoint-Zugriff (Abschnitt~2.5).

\subsection{SAP Business Technology Platform}

Die SAP \gls{btp} ist die zentrale Entwicklungs- und Integrationsplattform von SAP für Cloud-Anwendungen \citep{sap2024btp}. Sie vereint Datenbank-, Analyse-, Integrations- und Entwicklungsfunktionen in einer einheitlichen Umgebung und bildet damit die technologische Grundlage für die in dieser Arbeit entwickelte Middleware.

Die Plattform gliedert sich in vier Bereiche: Database \& Data Management für Datenhaltung und -verarbeitung (SAP HANA Cloud), Analytics für Auswertungen und Dashboards (SAP Analytics Cloud), Application Development \& Integration für die Anwendungsentwicklung (CAP, Integration Suite) sowie Intelligent Technologies für KI- und IoT-Szenarien. Für die vorliegende Arbeit ist der Bereich Application Development relevant, da die Middleware als CAP-Anwendung auf der BTP entwickelt und betrieben wird. Die BTP stellt dafür verschiedene Laufzeitumgebungen bereit, darunter Cloud Foundry, Kyma (Kubernetes-basiert) und ABAP Environment \citep{erl2013}.
...
Cloud Foundry organisiert Anwendungen in einer hierarchischen Struktur aus Organizations und Spaces, die eine Trennung von Umgebungen (z.\,B. Entwicklung, Test, Produktion) ermöglicht. Das Deployment nutzt Buildpacks, die die Programmiersprache automatisch erkennen und die passende Laufzeitumgebung bereitstellen -- für die BC Middleware ist dies das \texttt{nodejs\_buildpack}. Verwaltete Services wie Datenbanken oder Message Queues können an Anwendungen gebunden werden, und Routes definieren die URL, unter der eine Anwendung extern erreichbar ist.
...
CAP orientiert sich am Domain-Driven Design \citep{evans2003}: Der Entwickler modelliert zunächst die Domänenkonzepte (Entitäten, Services, Beziehungen) deklarativ in \gls{cds}, und das Framework generiert daraus automatisch die technische Infrastruktur -- OData-Endpunkte, Datenbankschemas und Authentifizierung. Dieser deklarative Ansatz reduziert den Boilerplate-Code erheblich und ermöglicht es, sich auf die fachliche Logik zu konzentrieren. Darüber hinaus abstrahiert CAP über verschiedene Datenbanken (SQLite, HANA, PostgreSQL), sodass dieselbe Anwendung in unterschiedlichen Umgebungen betrieben werden kann.
...
CDS definiert mehrere Konzepte, die in der Middleware Verwendung finden. Entities beschreiben Datenstrukturen mit Schlüsselfeldern und Attributen und bilden die Grundlage für die OData-Entitätsmengen. Services gruppieren mehrere Entities zu einer kohärenten \gls{api}, die unter einem gemeinsamen Pfad exponiert wird. Functions repräsentieren lesende Operationen ohne Seiteneffekte (HTTP GET), während Actions schreibende Operationen mit potentiellen Seiteneffekten abbilden (HTTP POST). Annotations ergänzen die Definitionen um Metadaten, etwa für UI-Rendering, Validierung oder Autorisierung.
...
\begin{tikzpicture}[
    ...
    \node[box] (odata) at (10,-1) {SAPAdapter};
...
Im Kontext dieser Arbeit ermöglicht das Adapter-Pattern den Austausch der Datenquelle (Excel vs. direkte SAP-Anbindung), ohne dass die darüberliegende Service-Schicht angepasst werden muss (siehe Abbildung~\ref{fig:adapter-pattern}). Der Adapter übersetzt die spezifische Schnittstelle der Datenquelle (Excel-Datei, SAP-APIs) in ein einheitliches internes Datenmodell.

\subsubsection{Repository Pattern}

Das Repository-Pattern abstrahiert den Datenzugriff und stellt eine einheitliche Schnittstelle für die Geschäftslogik bereit \citep{fowler2002}. Es fungiert als Vermittler zwischen der Domänenschicht und der Datenzugriffsschicht, indem es Methoden wie \texttt{findAll()}, \texttt{findById()} oder \texttt{save()} bereitstellt, hinter denen die konkrete Datenbeschaffung verborgen bleibt.

Der zentrale Vorteil besteht in der Entkopplung: Die Geschäftslogik arbeitet gegen eine abstrakte Schnittstelle und hat keine Kenntnis darüber, ob die Daten aus einer Datenbank, einer Datei oder einem externen Service stammen. Dies verbessert die Testbarkeit erheblich, da das Repository in Tests durch Mock-Objekte ersetzt werden kann, ohne die eigentliche Logik zu verändern. Gleichzeitig zentralisiert das Pattern die Datenzugriffslogik an einer Stelle, was Änderungen an der Datenquelle auf einen klar definierten Bereich begrenzt.

In der BC Middleware wird das Repository-Pattern konzeptionell mit dem Adapter-Pattern kombiniert: Die \texttt{loadData()}-Funktion übernimmt die Rolle des Repositories und delegiert den tatsächlichen Datenzugriff an den jeweiligen Adapter. Die Service-Schicht ruft ausschließlich \texttt{loadData()} auf und erhält ein einheitliches Datenobjekt, unabhängig davon, ob der Excel-Adapter oder ein zukünftiger SAP-Adapter die Daten liefert.
