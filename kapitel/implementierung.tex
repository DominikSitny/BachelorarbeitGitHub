...
Das Zurückschreiben in Excel-Dateien erwies sich als komplexer als ursprünglich angenommen. Die xlsx-Bibliothek unterstützt zwar das Schreiben, jedoch sind folgende Einschränkungen zu beachten:
...
Die Zeitbuchungen aus dem SAP-Export (CADO-Sheet) weisen ein spezielles Format auf:
...
Für die Frontend-Entwickler wurde eine interaktive API-Dokumentation erstellt, welche unter der Root-URL der Middleware erreichbar ist. Diese umfasst:
...
Zur Sicherstellung der Softwarequalität wurde eine umfassende Test-Suite mit dem JavaScript-Testframework Jest implementiert \citep{jest2024}. Die 77 automatisierten Tests gliedern sich in Unit- und Integrationstests:
...
\begin{itemize}
    \item Korrekte Aggregation der Budget-Metriken (Budget PT, Actual PT, ETC, EAC)
    \item Konvertierung von Stunden zu Personentagen (8h = 1 PT)
    \item Auflösung von PSP-Suffixen (z.B. \enquote{C72773-001.1} $\rightarrow$ \enquote{C72773-001})
    \item Session-Lifecycle und Timeout-Logik (8 Stunden)
    \item Parsing deutscher Zahlenformate (Komma als Dezimaltrennzeichen)
\end{itemize}
