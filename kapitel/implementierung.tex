\section{Implementierung}

In diesem Kapitel wird die Implementierung der BC Middleware auf der SAP \gls{btp} dokumentiert. Die Umsetzung folgt den in Kapitel~4 beschriebenen Architekturprinzipien und nutzt die in Kapitel~2 vorgestellten Technologien. Der Quellcode ist in einem Git-Repository versioniert und folgt etablierten Entwicklungspraktiken \citep{sommerville2015}.

\subsection{Projektstruktur}

Die Middleware folgt der Standard-Projektstruktur des SAP \gls{cap} \citep{sap2024cap}, die eine klare Trennung zwischen Service-Definition, Geschäftslogik und Testinfrastruktur vorsieht. Das Verzeichnis \texttt{srv/} enthält die gesamte Server-Logik, wobei die Bibliotheken in \texttt{srv/lib/} als eigenständige Module organisiert sind, um die in Kapitel~4 beschriebene Schichtentrennung auch auf Dateiebene abzubilden. Die Testdaten in \texttt{excel\_templates/} simulieren die SharePoint-Verzeichnisstruktur, sodass neue Projekte durch einfaches Hinzufügen eines Ordners verfügbar werden. Auflistung~\ref{lst:project-structure} zeigt die Struktur im Detail:

\begin{lstlisting}[caption={Projektstruktur}, label={lst:project-structure}]
bc_middleware/
  srv/
    budget-service.cds    # CDS Service Definition
    budget-service.js     # Service Implementation
    index.html            # API-Dokumentation
    lib/
      budget-calculations.js  # Budget-Metriken Berechnung
      excel-parser.js         # Excel Adapter
      ms-auth.js              # Microsoft Graph Auth
      sharepoint.js           # SharePoint Integration
  test/
    unit/                     # Unit-Tests (Jest)
    integration/              # Integrationstests
  excel_templates/
    Projekt_1/
      Budgetcontrolling.xlsx
  package.json
  manifest.yaml           # CF Deployment Config
\end{lstlisting}

\subsection{CDS Service-Definition}

Die Service-Definition in \gls{cds} bildet die zentrale Schnittstellenbeschreibung der Middleware. Aus ihr generiert SAP \gls{cap} automatisch einen vollständigen OData v4 Service mit Metadaten-Dokument, Filterung, Paginierung und Sortierung \citep{sap2024cap}. Auflistung~\ref{lst:cds-service} zeigt einen Auszug mit zwei exemplarischen Entitäten, einer Funktion und einer Action.

\begin{lstlisting}[caption={Auszug aus budget-service.cds}, label={lst:cds-service}]
service BudgetService @(path: '/api/budget') {

    @cds.persistence.skip
    entity PSPElements {
        key psp        : String(50);
            name       : String(200);
            team       : String(100);
            budgetPT   : Decimal(15, 2);
    }

    @cds.persistence.skip
    entity BudgetOverview {
        key team      : String(100);
            budgetPT  : Decimal(15, 2);
            actualPT  : Decimal(15, 2);
            etcPT     : Decimal(15, 2);
            eacPT     : Decimal(15, 2);
    }

    function getProjects() returns array of String;

    action startSession(project : String) returns {
        success   : Boolean;
        sessionId : String;
        project   : String;
    };
}
\end{lstlisting}

Die Annotation \texttt{@cds.persistence.skip} signalisiert, dass die Entitäten nicht in einer Datenbank persistiert werden, sondern zur Laufzeit aus den Excel-Daten generiert werden.

\subsection{Excel-Adapter}

Der Excel-Adapter ist das Herzstück der aktuellen Backend-Implementierung. Er nutzt die \texttt{xlsx}-Bibliothek (SheetJS) zum Parsen und Schreiben von Excel-Dateien \citep{sheetjs2024}. Die Bibliothek unterstützt verschiedene Formate (XLSX, XLS, CSV) und ermöglicht sowohl lesenden als auch schreibenden Zugriff. Die Wahl einer JavaScript-Bibliothek für die Excel-Verarbeitung folgt dem Paradigma, die gesamte Server-Logik in einer einzigen Laufzeitumgebung zu halten \citep{nodejs2024}.

\subsubsection{Datenextraktion}

\begin{lstlisting}[caption={Excel-Parser Hauptfunktion}, label={lst:excel-parser}]
function loadNewStructure(basePath, project) {
    const budgetPath = path.join(basePath, project,
        'Budgetcontrolling.xlsx');
    const budgetData = parseBudgetcontrolling(budgetPath);

    // PSP-Overrides aus "1. PSP-Zuordnung" anwenden
    const userStories = applyPSPOverrides(
        budgetData.userStories,
        budgetData.pspZuordnungUserStory
    );

    return {
        teams: budgetData.prueftabellen.teams,
        pspElemente: budgetData.pspElemente,
        userStories: userStories,
        timeBookings: budgetData.timeBookings,
        // ... weitere Daten
    };
}
\end{lstlisting}

\subsubsection{Sheet-Parsing}

Jedes Sheet wird individuell geparst und auf das einheitliche Datenmodell gemappt:

\begin{lstlisting}[caption={Parsing der PSP-Elemente}, label={lst:parse-psp}]
function parsePSPElements(workbook) {
    const data = XLSX.utils.sheet_to_json(
        workbook.Sheets['PSP-Elemente']
    );

    return data.map(row => ({
        psp: row['PSP'] || '',
        name: row['Name'] || '',
        pspElement: row['PSP-Element'] || '',
        team: row['Team'] || '',
        budgetPT: parseFloat(row['Budget [PT]']) || 0
    }));
}
\end{lstlisting}

\subsubsection{Write-Back für PSP-Zuordnungen}

\gls{crud}-Operationen für \gls{psp}-Zuordnungen werden direkt in die Excel-Datei geschrieben:

\begin{lstlisting}[caption={PSP-Zuordnung hinzufügen}, label={lst:add-assignment}]
function addPSPAssignment(filePath, assignment) {
    const workbook = XLSX.readFile(filePath);
    const sheet = workbook.Sheets['1. PSP-Zuordnung'];
    const data = XLSX.utils.sheet_to_json(sheet);

    // Duplikat pruefen
    const exists = data.some(row =>
        row['User-Story'] === assignment.userStory &&
        row['PSP'] === assignment.psp
    );
    if (exists) throw new Error('Zuordnung existiert bereits');

    // Neue Zeile hinzufuegen
    data.push({
        'User-Story': assignment.userStory,
        'PSP': assignment.psp,
        'Team User-Story': assignment.teamUserStory,
        'Team PSP': assignment.teamPSP
    });

    // Sheet neu schreiben und speichern
    workbook.Sheets['1. PSP-Zuordnung'] =
        XLSX.utils.json_to_sheet(data);
    XLSX.writeFile(workbook, filePath);
}
\end{lstlisting}

\subsection{Budget-Metriken Berechnung}

Die Budget-Metriken werden zur Laufzeit aus den geladenen Daten berechnet. Die Berechnungslogik ist in ein eigenständiges Modul (\texttt{budget-calculations.js}) ausgelagert, das keine Abhängigkeit zur Datenquelle besitzt -- es arbeitet ausschließlich auf den vom Adapter bereitgestellten Datenstrukturen. Dadurch kann das Modul isoliert getestet werden und erreicht eine Testabdeckung von 100\,\%. Die vier Metriken Budget, Actual, \gls{etc} und \gls{eac} werden jeweils in zwei Aggregationsebenen berechnet: pro Team und pro \gls{psp}-Element.

\subsubsection{Budget pro Team}

\begin{lstlisting}[caption={Budget-Berechnung pro Team}, label={lst:budget-calc}]
function calculateBudgetByTeam(pspElemente) {
    const budgetByTeam = {};

    for (const elem of pspElemente) {
        const team = elem.team || '(Leer)';
        if (!budgetByTeam[team]) budgetByTeam[team] = 0;
        budgetByTeam[team] += elem.budgetPT || 0;
    }

    return budgetByTeam;
}
\end{lstlisting}

\subsubsection{ETC-Berechnung}

Die \gls{etc}-Berechnung berücksichtigt nur User Stories mit gültiger \gls{psp}-Zuordnung:

\begin{lstlisting}[caption={ETC-Berechnung}, label={lst:etc-calc}]
function calculateEtcByTeam(userStories) {
    const etcByTeam = {};

    for (const story of userStories) {
        const psp = story.pspElement || '';
        // Nur Stories mit PSP-Zuordnung zaehlen
        if (!psp || psp === 'unassigned') continue;

        const team = story.team || '(Leer)';
        if (!etcByTeam[team]) etcByTeam[team] = 0;
        etcByTeam[team] += story.etcPT || 0;
    }

    return etcByTeam;
}
\end{lstlisting}

\subsection{Session-Management}

Die Implementierung des in Abschnitt~4.4 entworfenen Session-Managements nutzt eine JavaScript \texttt{Map} als In-Memory-Speicher. Die Session-ID wird aus einer Kombination von Zufallswert und Zeitstempel generiert, was Eindeutigkeit ohne externe Abhängigkeiten sicherstellt. Die Zuordnung von Session zu Projekt wird bei jedem Request über den \texttt{x-session-id}-Header aufgelöst. Auflistung~\ref{lst:session-mgmt} zeigt die Kernfunktionen:

\begin{lstlisting}[caption={Session-Verwaltung}, label={lst:session-mgmt}]
const sessions = new Map();

function startSession(project) {
    const sessionId = Math.random().toString(36).substring(2)
        + Date.now().toString(36);
    sessions.set(sessionId, {
        project,
        createdAt: new Date().toISOString()
    });
    return sessionId;
}

function getSession(sessionId) {
    return sessions.has(sessionId)
        ? sessions.get(sessionId) : null;
}
\end{lstlisting}

\subsection{Deployment}

Die Middleware wird auf SAP \gls{btp} Cloud Foundry deployed. Die Deployment-Konfiguration erfolgt über eine \texttt{manifest.yaml}, die den Applikationsnamen, die zugewiesene Speichergröße, das Buildpack und die Route definiert (Auflistung~\ref{lst:manifest}). Mit 256\,MB Arbeitsspeicher ist die Instanz für die erwartete Nutzerzahl ausreichend dimensioniert. Das \texttt{nodejs\_buildpack} stellt die Node.js-Laufzeitumgebung bereit, in der SAP \gls{cap} den OData-Service startet.

\begin{lstlisting}[caption={manifest.yaml}, label={lst:manifest}]
applications:
  - name: budget-middleware
    memory: 256M
    buildpacks:
      - nodejs_buildpack
    command: npm start
    routes:
      - route: xxx-srv.cfapps.eu10.hana.ondemand.com
\end{lstlisting}

Das Deployment selbst erfolgt mit dem Cloud Foundry CLI in zwei Schritten -- Authentifizierung und Push:

\begin{lstlisting}[caption={Deployment-Befehle}, label={lst:deploy}]
cf login -a https://api.cf.eu10.hana.ondemand.com
cf push
\end{lstlisting}

\subsection{Herausforderungen bei der Implementierung}

Während der Implementierung traten mehrere Herausforderungen auf, welche im Folgenden dokumentiert werden:

\subsubsection{Excel-Formatierung beim Schreiben}

Das Zurückschreiben in Excel-Dateien erwies sich als komplexer als ursprünglich angenommen. Die xlsx-Bibliothek unterstützt zwar das Schreiben, jedoch sind folgende Einschränkungen zu beachten: Formeln in anderen Zellen werden nicht automatisch neu berechnet, Zellenformatierungen wie Farben und Rahmen müssen explizit erhalten werden und die Power-Query-Verbindungen im Excel bleiben unberührt.

Anstatt einzelne Zellen zu manipulieren und dabei die Formatierung zu erhalten, wurde ein pragmatischerer Ansatz gewählt: Das betroffene Sheet wird mit \texttt{json\_to\_sheet} vollständig neu geschrieben (vgl. Auflistung~\ref{lst:add-assignment}). Dieser Ansatz ist robuster, da er keine Abhängigkeit von der bestehenden Zellstruktur hat, jedoch geht die Formatierung des \enquote{1. PSP-Zuordnung}-Sheets dabei verloren -- ein akzeptabler Trade-off, da dieses Sheet primär maschinell verarbeitet wird.

\subsubsection{CADO-Zeitbuchungen parsen}

Die Zeitbuchungen aus dem SAP-Export (CADO-Sheet) weisen ein spezielles Format auf: Das deutsche Zahlenformat verwendet das Komma als Dezimaltrennzeichen und Datumsangaben folgen dem Format DD.MM.YYYY. Zudem enthalten die PSP-Elemente Suffixe (z.\,B. \enquote{C72773-001.1}), die bei der Budget-Berechnung auf den Basis-PSP (z.\,B. \enquote{C72773-001}) aufgelöst werden müssen, um eine korrekte Zuordnung zu den PSP-Elementen zu ermöglichen.

\begin{lstlisting}[caption={Parsing der CADO-Zeitbuchungen}, label={lst:cado-parse}]
function parseCADO(workbook) {
    const sheet = workbook.Sheets['CADO'];
    const rawData = XLSX.utils.sheet_to_json(sheet,
        { header: 1 });
    const timeBookings = [];

    // Daten starten ab Zeile 6 (Index 5)
    for (let i = 5; i < rawData.length; i++) {
        const row = rawData[i];
        if (!row || row.length < 8) continue;

        const belegnummer = String(row[2] || '').trim();
        // Nur Zeilen mit gueltiger Belegnummer
        if (!/^\d{8}$/.test(belegnummer)) continue;

        // Deutsches Zahlenformat: "2,500" -> 2.5
        const anzahl = parseFloat(
            String(row[5] || '').replace(',', '.')
        ) || 0;

        timeBookings.push({
            belegnummer,
            leistungsart: String(row[3] || ''),
            datum: String(row[4] || ''),
            anzahl,
            pspElement: String(row[7] || '').trim()
        });
    }
    return timeBookings;
}
\end{lstlisting}

\subsubsection{Konkurrierende Schreibzugriffe}

Ein inhärentes Problem des Excel-basierten Backends sind konkurrierende Schreibzugriffe. Wenn zwei Benutzer gleichzeitig \gls{psp}-Zuordnungen erstellen, können Race Conditions auftreten: Beide lesen den aktuellen Stand der Datei, fügen ihre Änderung hinzu und schreiben zurück -- wobei die zweite Schreiboperation die Änderung der ersten überschreibt (\textit{Lost Update Problem}) \citep{tanenbaum2017}.

Die aktuelle Implementierung adressiert dieses Problem nicht vollständig. Node.js ist zwar single-threaded, sodass innerhalb einer Server-Instanz keine echte Parallelität auftritt, jedoch können bei horizontaler Skalierung (mehrere Cloud-Foundry-Instanzen) oder bei direkter Bearbeitung der Excel-Datei durch Benutzer Konflikte entstehen.

Mögliche Lösungsansätze für den produktiven Einsatz wären:
\begin{itemize}
    \item \textbf{Optimistic Locking}: Speichern eines Zeitstempels oder einer Versionsnummer, Ablehnung bei Konflikt
    \item \textbf{Pessimistic Locking}: Exklusive Dateisperre während des Schreibvorgangs (problematisch bei SharePoint)
    \item \textbf{Event Sourcing}: Speichern von Änderungen als Events statt direkter Dateimanipulation
\end{itemize}

Für den Prototyp ist die aktuelle Lösung akzeptabel, da die erwartete Nutzerzahl gering ist (< 50) und gleichzeitige Schreibzugriffe selten auftreten. Bei einem Wechsel zum SAP-Backend entfällt dieses Problem, da die SAP-Systeme ACID-Transaktionen unterstützen.

\subsubsection{Session-Validierung in jedem Request}

Jeder API-Request (mit Ausnahme von \texttt{getProjects} und \texttt{startSession}) erfordert eine Session-Validierung. Diese wird vor der eigentlichen Handler-Logik geprüft:

\begin{lstlisting}[caption={Session-Validierung}, label={lst:session-validation}]
function getSessionFromRequest(req) {
    const sessionId = req.headers?.['x-session-id']
        || req._.req?.headers?.['x-session-id'];

    if (sessionId && sessions.has(sessionId)) {
        const session = sessions.get(sessionId);

        // Session-Timeout pruefen (8 Stunden)
        const age = Date.now()
            - new Date(session.createdAt).getTime();
        if (age > 8 * 60 * 60 * 1000) {
            sessions.delete(sessionId);
            return null;
        }
        return session;
    }
    return null;
}

function requireSession(req) {
    const project = getProjectFromRequest(req);
    if (!project) {
        req.error(401, 'Keine aktive Session.');
        return null;
    }
    return project;
}
\end{lstlisting}

\subsection{API-Dokumentation}

Da das Fiori-Frontend von anderen Studierenden entwickelt wird (vgl. Abschnitt~\ref{sec:abgrenzung}), ist eine vollständige und aktuelle API-Dokumentation essentiell für die Zusammenarbeit. Anstatt eine separate Dokumentation zu pflegen, die schnell veraltet, wurde eine interaktive HTML-Seite (\texttt{index.html}) direkt im \texttt{srv/}-Verzeichnis erstellt, die unter der Root-URL der Middleware erreichbar ist. Diese Dokumentation umfasst eine Übersicht aller Endpunkte mit Beschreibung, Request/Response-Beispiele für jeden Endpunkt, eine Erklärung der Session-Authentifizierung sowie OData Query-Optionen mit Beispielen. Da die Dokumentation im selben Repository wie der Code liegt, wird sie bei jedem Deployment automatisch mit aktualisiert.

\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{
\textbf{API Dokumentation - BC Middleware}\\[0.5em]
\texttt{Base URL: /api/budget}\\[1em]
\textbf{Session Endpunkte}\\
\texttt{GET /getProjects()} -- Liste verfügbarer Projekte\\
\texttt{POST /startSession} -- Session starten\\[0.5em]
\textbf{Daten Endpunkte} (benötigen x-session-id Header)\\
\texttt{GET /Teams} -- Alle Projekt-Teams\\
\texttt{GET /BudgetOverview} -- Budget-Metriken pro Team\\
...
}}
\caption{Auszug aus der API-Dokumentation (index.html)}
\label{fig:api-doc}
\end{figure}

\subsection{Testdaten}

Für Entwicklung und Test wurde ein Beispielprojekt \enquote{Projekt\_1} mit folgenden Daten angelegt:

\begin{table}[H]
\centering
\caption{Testdaten im Projekt\_1}
\label{tab:testdata}
\begin{tabularx}{\textwidth}{Xcc}
\toprule
\textbf{Entität} & \textbf{Anzahl} & \textbf{Quelle} \\
\midrule
Teams & 23 & Prüftabellen-Sheet \\
Timeboxes & 5 & Prüftabellen-Sheet \\
PSP-Elemente & 113 & PSP-Elemente-Sheet \\
User Stories & 251 & qry\_CALM\_Data-Sheet \\
Zeitbuchungen & 3.555 & CADO-Sheet \\
PSP-Zuordnungen & 4 & 1. PSP-Zuordnung-Sheet \\
\bottomrule
\end{tabularx}
\end{table}

Diese Testdaten (siehe Tabelle~\ref{tab:testdata}) ermöglichen realistische Tests aller Funktionen, einschließlich der Budget-Berechnungen und PSP-Zuordnungen. Die Datenmengen entsprechen einem typischen mittelgroßen SAP-Implementierungsprojekt.

\subsection{Qualitätssicherung durch automatisierte Tests}

Zur Sicherstellung der Softwarequalität wurde eine umfassende Test-Suite mit dem JavaScript-Testframework Jest implementiert \citep{jest2024}. Die 113 automatisierten Tests gliedern sich in Unit- und Integrationstests:

\begin{table}[H]
\centering
\caption{Übersicht der automatisierten Tests}
\label{tab:tests}
\begin{tabularx}{\textwidth}{Xcc}
\toprule
\textbf{Kategorie} & \textbf{Anzahl} & \textbf{Typ} \\
\midrule
Budget-Berechnungen & 52 & Unit \\
Session-Management & 13 & Unit \\
Excel-Parser & 9 & Unit \\
API-Endpoints & 39 & Integration \\
\midrule
\textbf{Gesamt} & \textbf{113} & \\
\bottomrule
\end{tabularx}
\end{table}

Die Budget-Berechnungslogik wurde in ein separates Modul (\texttt{budget-calculations.js}) extrahiert, um eine bessere Testbarkeit zu erreichen. Dieses Modul erreicht eine Testabdeckung von 100\,\%.

\subsubsection{Unit-Tests}

Die Unit-Tests prüfen isolierte Funktionen ohne externe Abhängigkeiten:

\begin{lstlisting}[caption={Beispiel Unit-Test für Budget-Berechnung}, label={lst:unit-test}]
describe('calculateBudgetByTeam', () => {
    it('sollte Budget pro Team korrekt aggregieren', () => {
        const pspElemente = [
            { psp: 'PSP-001', team: 'Team A', budgetPT: 100 },
            { psp: 'PSP-002', team: 'Team A', budgetPT: 50 },
            { psp: 'PSP-003', team: 'Team B', budgetPT: 200 }
        ];

        const result = calculateBudgetByTeam(pspElemente);

        expect(result['Team A']).toBe(150);
        expect(result['Team B']).toBe(200);
    });
});
\end{lstlisting}

Die Tests validieren unter anderem die korrekte Aggregation der Budget-Metriken (Budget PT, Actual PT, ETC, EAC), die Konvertierung von Stunden zu Personentagen (8h = 1 PT), die Auflösung von PSP-Suffixen (z.B. \enquote{C72773-001.1} $\rightarrow$ \enquote{C72773-001}), den Session-Lifecycle einschließlich der Timeout-Logik (8 Stunden) sowie das Parsing deutscher Zahlenformate mit dem Komma als Dezimaltrennzeichen.

\subsubsection{Integrationstests}

Die Integrationstests prüfen das Zusammenspiel der Komponenten mit realen Testdaten aus der Budgetcontrolling.xlsx:

\begin{lstlisting}[caption={Beispiel Integrationstest}, label={lst:integration-test}]
describe('GET /BudgetOverview', () => {
    it('sollte Budget-Metriken pro Team berechnen', () => {
        const pspToTeam = createPspToTeamMapping(testData.pspElemente);
        const actualByTeam = calculateActualByTeam(
            testData.timeBookings, pspToTeam
        );

        // Gesamte Actual PT sollte positiv sein
        const totalActual = Object.values(actualByTeam)
            .reduce((a, b) => a + b, 0);
        expect(totalActual).toBeGreaterThan(0);
    });
});
\end{lstlisting}

Die Tests werden mit folgenden Befehlen ausgeführt:

\begin{lstlisting}[caption={Test-Befehle}, label={lst:test-commands}]
npm test              # Alle Tests ausfuehren
npm run test:watch    # Tests im Watch-Modus
npm run test:coverage # Coverage-Report generieren
\end{lstlisting}
